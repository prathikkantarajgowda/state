...
state_2000 <- 
  read_csv("data/2000/nhgis0042_csv/nhgis0042_ds146_2000_state.csv") %>% 
  as_tibble() %>%
  ...
  transmute(country = "United States", 
            state,
            gender = ifelse(gender_num == 0 | gender_num > 23,
                            "female", "male"),
            race = case_when(race_num <= 1 ~ "white",
                             race_num <= 2 ~ "black_africanamerican",
                             race_num <= 3 ~ "native",
                             race_num <= 4 ~ "asian",
                             race_num <= 5 ~ "nativehawaiian_pacificislander",
                             race_num <= 6 ~ "other",
